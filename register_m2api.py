...
引擎: M2API免费  |  2次过盾/账号  (signup + login)
...
# ── Turnstile (via self-hosted M2API solver — FREE) ───
def solve_turnstile(timeout=120):
    resp = requests.post(f"{M2API_URL}/createTask", json={
        "task": {"type": "TurnstileTaskProxyless", "websiteURL": SITE_URL, "websiteKey": SITEKEY}
    }, timeout=15)
    data = resp.json()
    if data.get("errorId") != 0:
        raise RuntimeError(f"M2API createTask failed: {data}")
    task_id = data["taskId"]
    start = time.time()
    while time.time() - start < timeout:
        time.sleep(2)
        resp = requests.post(f"{M2API_URL}/getTaskResult", json={
            "taskId": task_id
        }, timeout=15)
        data = resp.json()
        if data.get("status") == "ready":
            log(f"  Turnstile solved ({time.time()-start:.1f}s) [FREE]")
            return data["solution"]["token"]
        if data.get("status") == "CAPTCHA_FAIL":
            raise RuntimeError(f"M2API solve failed: {data}")
    ...
    for attempt in range(5):
        try:
            ...
            if resp.status_code != 201:
                ...
                log(f"  mail.tm 创建失败 (attempt {attempt+1}): {resp.status_code} {error_text}", "WARN")
                time.sleep(3)
                continue
...
def main():
    ...
    # Create the key — response contains full key in a flash/alert message
    resp = session.post(f"{SITE_URL}/account/api-keys/keys", data={
        "csrf_token": csrf_token,
        "name": "my-api-key"
    }, headers={"x-csrf-token": csrf_token}, allow_redirects=False)

    # Extract full API key from response
    # Pattern: the key appears in <code> tag without ellipsis (…)
    api_key = None
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="autobricksai.com 全自动注册")
